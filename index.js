...
* In node js things are checked 1 by one means 
    ...
    means the first matching thing (URL) will executed.

    app.use is indepedent method (get,post,delete)

    **sequence in node and express is very important of (url like post,get etc ) 
...
app.get("/todos/:idss", (req, res) => {
    //   / => REQ.PARAMS USE MADADU
    console.log(req.params);
    if(req.params.id){
        const todos=dummyTodosData.find((todo) => req.params.id==todo.id);
        res.json({
        msg: "todos get api",
        result: [todos]
    });
...
app.get("/users",(req,res) => {
    // in POSTMAN in URL user?id=10 then to get that id we use Req.QUERY
